from core.retry_guardrails import get_max_l2_llm_attempts, get_max_llm_attempts, parse_positive_int, should_retry_llm
...
def test_l2_retry_defaults_to_two_and_is_capped(monkeypatch):
    monkeypatch.delenv("L2_LLM_MAX_ATTEMPTS", raising=False)
    assert get_max_l2_llm_attempts() == 2
    monkeypatch.setenv("L2_LLM_MAX_ATTEMPTS", "20")
    assert get_max_l2_llm_attempts() == 3
